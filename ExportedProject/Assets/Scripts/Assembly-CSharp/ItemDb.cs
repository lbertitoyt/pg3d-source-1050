...
public static class ItemDb
{
	...
	public static ShopPositionParams GetInfoForNonWeaponItem(string name, ShopNGUIController.CategoryNames category)
	{
		...
		{
			result = Resources.Load<ShopPositionParams>("Armor/" + name);
		}
		if (category == ShopNGUIController.CategoryNames.HatsCategory)
		{
			result = Resources.Load<ShopPositionParams>("Hats/" + name);
		}
		if (category == ShopNGUIController.CategoryNames.CapesCategory)
		{
			result = Resources.Load<ShopPositionParams>("Capes/" + name);
		}
		if (category == ShopNGUIController.CategoryNames.BootsCategory)
		{
			result = Resources.Load<ShopPositionParams>("Shop_Boots/" + name);
		}
		...
		{
			result = Resources.Load<ShopPositionParams>("Masks/" + name);
		}
		return result;
	}
		...
		if (byTag != null)
		{
			...
			if (weaponSounds != null)
			{
				try
				{
					...
					ItemRecord firstRec = GetByTag(tag2);
					return WeaponManager.AllWrapperPrefabs().First((WeaponSounds weapon) => weapon.name == firstRec.PrefabName).shopName;
				}
				catch (Exception ex)
				{
	...
	public static string GetItemNameNonLocalized(string tag, string shopId, ShopNGUIController.CategoryNames category, string defaultDesc = null)
	{
		...
		{
			ShopPositionParams shopPositionParams = Resources.Load<ShopPositionParams>("Hats/" + tag);
