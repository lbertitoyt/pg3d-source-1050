...
public class LeaguesGUIController : MonoBehaviour
{
	...
	private void SetInfoFromLeague(RatingSystem.RatingLeague league)
	{
		...
		else
		{
			...
			_lblScore.text = ((league != (RatingSystem.RatingLeague)RiliExtensions.EnumNumbers<RatingSystem.RatingLeague>().Max()) ? string.Format("{0}/{1}", RatingSystem.instance.currentRating, num2) : RatingSystem.instance.currentRating.ToString());
