...
public sealed class InGameGUI : MonoBehaviour
{
	...
	public GameObject bankViewPause;

	public GameObject bankViewPauseLow;
	...
	private void Start()
	{
		...
		bankViewPause.SetActive(!Device.isPixelGunLow);
		bankViewPauseLow.SetActive(Device.isPixelGunLow);
	...
	private void Update()
	{
		...
		if (isMulti && playerMoveC != null)
		{
			...
			else
			{
				...
				if (timerShowScorePict > 0f)
				{
					if (!multyKillPanel.gameObject.activeSelf || scorePictName != multyKillSprite.spriteName)
					{
		...
		if (!Defs.isDaterRegim && Defs.isMulti && ConnectSceneNGUIController.regim == ConnectSceneNGUIController.RegimGame.Deathmatch)
		{
