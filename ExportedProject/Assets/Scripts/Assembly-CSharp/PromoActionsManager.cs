...
public sealed class PromoActionsManager : MonoBehaviour
{
	...
	public class MobileAdvertInfo
	{
		...
		private string _awardCoinsPayningCurrency = "Coins";

		private int _awardCoinsNonpayning = 1;

		private string _awardCoinsNonpayningCurrency = "Coins";
		...
		public string AwardCoinsPayingCurrency
		{
			get
			{
				return _awardCoinsPayningCurrency;
			}
			set
			{
				_awardCoinsPayningCurrency = ((!(value == "Coins") && !(value == "GemsCurrency")) ? "Coins" : value);
			}
		}
		...
		public string AwardCoinsNonpayingCurrency
		{
			get
			{
				return _awardCoinsNonpayningCurrency;
			}
			set
			{
				_awardCoinsNonpayningCurrency = ((!(value == "Coins") && !(value == "GemsCurrency")) ? "Coins" : value);
			}
		}
	...
	private void Start()
	{
		...
		System.Threading.Tasks.Task futureToWait = PersistentCacheManager.Instance.StartDownloadSignaturesLoop();
	...
	private IEnumerator GetActionsLoop(System.Threading.Tasks.Task futureToWait)
	{
	...
	private IEnumerator GetAdvertInfoLoop(System.Threading.Tasks.Task futureToWait)
	{
		System.Threading.Tasks.Task futureToWait2 = default(System.Threading.Tasks.Task);
		yield return new WaitUntil(() => futureToWait2.IsCompleted);
	...
	private IEnumerator GetEventX3Info()
	{
		...
		while (!FriendsController.isInitPixelbookSettingsFromServer)
		{
			yield return null;
		}
		...
		if (advertInfo.ContainsKey("mobileAdvert_list"))
		{
			...
			if (mobileAdvertList != null)
			{
				...
				if (mobileAdvert != null)
				{
					...
					object awardCoinsPayngCurrency;
					if (mobileAdvert.TryGetValue("awardCurrencyPaying", out awardCoinsPayngCurrency))
					{
						_mobileAdvert.AwardCoinsPayingCurrency = awardCoinsPayngCurrency.ToString();
					}
					else
					{
						Debug.Log("mobileAdvert doesn't contain “awardCurrencyPaying” field. Default value is Coins");
					}
					...
					object awardCoinsNonpayingCurrency;
					if (mobileAdvert.TryGetValue("awardCurrencyNonPaying", out awardCoinsNonpayingCurrency))
					{
						_mobileAdvert.AwardCoinsNonpayingCurrency = awardCoinsNonpayingCurrency.ToString();
					}
					else
					{
						Debug.Log("mobileAdvert doesn't contain “awardCurrencyNonPaying” field. Default value is Coins");
					}
					object adProvider;
	...
	public IEnumerator GetActions()
	{
		...
		if (!string.IsNullOrEmpty(cachedResponse))
		{
			PersistentCacheManager.DebugReportCacheHit(promoActionAddress);
			responseText = cachedResponse;
		}
		else
		{
			PersistentCacheManager.DebugReportCacheMiss(promoActionAddress);
		...
		if (actions.TryGetValue("discounts_up", out discountsObj))
		{
			...
			{
				foreach (List<object> discount in discountsObjList)
				{
					if (discount.Count >= 2)
					{
		...
		object newsObj;
		if (actions.TryGetValue("news_up", out newsObj))
		{
			List<object> newsObjList = newsObj as List<object>;
			if (newsObjList != null)
			{
				foreach (string tg2 in newsObjList)
				{
					news.Add(tg2);
				}
			}
	...
	private IEnumerator GetBestBuyInfoLoop(System.Threading.Tasks.Task futureToWait)
	{
		System.Threading.Tasks.Task futureToWait2 = default(System.Threading.Tasks.Task);
		yield return new WaitUntil(() => futureToWait2.IsCompleted);
		while (true)
