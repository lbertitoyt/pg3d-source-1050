...
public class UICamera : MonoBehaviour
{
	...
	public void ProcessTouches()
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			Raycast(currentTouch);
			if (Input.touchPressureSupported && Defs.isUse3DTouch && currentTouch.current != null)
			{
				Notify(currentTouch.current, "OnPressure", currentTouch.pressure / currentTouch.maxPressure);
			}
			if (flag)
			{
				currentTouch.pressedCam = currentCamera;
