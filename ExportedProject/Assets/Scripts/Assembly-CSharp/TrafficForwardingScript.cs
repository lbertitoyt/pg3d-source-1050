...
internal sealed class TrafficForwardingScript : MonoBehaviour
{
	...
	internal IEnumerator GetTrafficForwardingConfigCoroutine()
	{
		...
		if (responseDict == null)
		{
			...
			_trafficForwardingPromise.TrySetException((Exception)new InvalidOperationException("Couldnot deserialize response: " + responseText));
		}
		else if (responseDict.TryGetValue("trafficForwarding_v_10.2.0", out trafficForwardingObject))
		{
			...
			else
			{
				...
				_trafficForwardingPromise.TrySetException((Exception)new InvalidOperationException("Couldnot deserialize trafficForwarding node: " + Json.Serialize(trafficForwardingObject)));
			}
		}
		else
		{
