...
public class TouchPadInJoystick : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (inGameGUI.playerMoveC != null && isActiveFireButton && (TrainingController.TrainingCompleted || TrainingController.CompletedTrainingStage > TrainingController.NewTrainingCompletedStage.None || TrainingController.FireButtonEnabled) && (!Defs.isHunger || _hungerGameController.isGo))
		{
			if (isShooting)
			{
			...
			{
				if (inGameGUI.playerMoveC.isShootingLoop && !JoystickController.rightJoystick.isShooting && !JoystickController.rightJoystick.isShootingPressure)
				{
					inGameGUI.playerMoveC.ShotUnPressed();
				}
				if (!JoystickController.rightJoystick.isShooting && !JoystickController.rightJoystick.isShootingPressure)
				{
					inGameGUI.playerMoveC.ResetShootingBurst();
				}
			}
		}
	...
	{
		if (Input.touchPressureSupported && Defs.isUse3DTouch && pressure > 0.8f)
		{
			if (!pressured)
			{
				pressured = true;
