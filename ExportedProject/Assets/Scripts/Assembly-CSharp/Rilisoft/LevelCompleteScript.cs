...
namespace Rilisoft
{
	public sealed class LevelCompleteScript : MonoBehaviour
	{
		...
		private void GiveAwardForCampaign()
		{
			...
			int num4 = 0;
			if (_starCount == 3 && _oldStarCount < 3)
			{
				num4 += 5;
			}
		...
		private void Start()
		{
			...
			_instance = this;
			if ((!Defs.IsSurvival && (_awardConferred || _hasAwardForMission)) || (_starCount == 3 && _oldStarCount < 3 && !_gameOver))
			{
			...
			else
			{
				...
				if (GlobalGameController.HasSurvivalRecord)
				{
					...
					if ((TwitterController.TwitterSupported) && !Device.isPixelGunLow)
					{
						mainInterface.SetActive(false);
