	...
	internal sealed class CloudCleaner
	{
		...
		public void CleanSavedGameFile(string filename)
		{
			...
			Action<ISavedGameMetadata> commit = delegate(ISavedGameMetadata metadata)
			{
				...
				PlayGamesPlatform.Instance.SavedGame.CommitUpdate(metadata, updateForMetadata, updatedBinaryData, delegate(SavedGameRequestStatus writeStatus, ISavedGameMetadata closeMetadata)
				{
