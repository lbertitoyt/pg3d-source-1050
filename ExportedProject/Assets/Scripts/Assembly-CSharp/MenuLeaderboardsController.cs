...
internal sealed class MenuLeaderboardsController : MonoBehaviour
{
	...
	private void TransitToFallbackState()
	{
		...
		leaderboardItemViewModel.WinCount = ((!LeaderboardScript.NewLeaderboards) ? PlayerPrefs.GetInt("TotalWinsForLeaderboards", 0) : RatingSystem.instance.currentRating);
	...
	private IEnumerable<float> FillListsWithResponseTextAsync(string responseText)
	{
		...
		LeaderboardItemViewModel selfStats = new LeaderboardItemViewModel
		{
			...
			WinCount = ((!LeaderboardScript.NewLeaderboards) ? PlayerPrefs.GetInt("TotalWinsForLeaderboards", 0) : RatingSystem.instance.currentRating),
			Highlight = true
		};
		...
		for (int k = orderedBestPlayersList.Count; k < MenuLeaderboardsView.PageSize; k++)
		{
		...
		for (int j = orderedClansList.Count; j < MenuLeaderboardsView.PageSize; j++)
		{
