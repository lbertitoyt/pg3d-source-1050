...
namespace GooglePlayGames.Native
{
	public class NativeClient : IPlayGamesClient
	{
		...
		{
			mServices.StatsManager().FetchForPlayer(delegate(GooglePlayGames.Native.PInvoke.StatsManager.FetchForPlayerResponse playerStatsResponse)
			{
				CommonStatusCodes responseCode = ConversionUtils.ConvertResponseStatusToCommonStatus(playerStatsResponse.Status());
				if (responseCode != 0 && responseCode != CommonStatusCodes.SuccessCached)
				{
				...
				if (callback != null)
				{
					...
					{
						GooglePlayGames.BasicApi.PlayerStats stats = playerStatsResponse.PlayerStats().AsPlayerStats();
						PlayGamesHelperObject.RunOnGameThread(delegate
						{
					...
					else
					{
						PlayGamesHelperObject.RunOnGameThread(delegate
						{
							callback(responseCode, new GooglePlayGames.BasicApi.PlayerStats());
						});
					}
				}
			});
