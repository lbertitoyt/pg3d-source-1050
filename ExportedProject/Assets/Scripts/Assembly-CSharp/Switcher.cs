...
internal sealed class Switcher : MonoBehaviour
{
	...
	private IEnumerator ParseConfigsCoroutine()
	{
		...
		ScopeLogger scopeLogger2 = new ScopeLogger("Switcher.Start()", "Parsing rating config", Defs.IsDeveloperBuild);
		try
		{
			if (!Storager.hasKey("ratSysConfigKey"))
			{
				Storager.setString("ratSysConfigKey", string.Empty, false);
			}
		}
		finally
		{
