...
internal sealed class ClanIncomingInvitesController : MonoBehaviour
{
	...
	internal static System.Threading.Tasks.Task<List<object>> RequestClanIncomingInvitesAsync(string playerId, float delay, CancellationToken ct)
	{
		...
		TaskCompletionSource<List<object>> val = new TaskCompletionSource<List<object>>();
		FriendsController.sharedController.StartCoroutine(RequestClanIncomingInvitesCoroutine(playerId, delay, val, ct));
		return val.Task;
	}

	internal static System.Threading.Tasks.Task<List<object>> RequestClanIncomingInvitesAsync(string playerId, CancellationToken ct)
	{
		//IL_0006: Unknown result type (might be due to invalid IL or missing references)
		return RequestClanIncomingInvitesAsync(playerId, 0f, ct);
	}

	internal static System.Threading.Tasks.Task<List<object>> RequestClanIncomingInvitesAsync(string playerId)
	{
		//IL_0006: Unknown result type (might be due to invalid IL or missing references)
	...
	private IEnumerator Start()
	{
		...
		{
			Debug.LogException((Exception)(object)CurrentRequest.Exception);
		}
